...
class NullTestUI():
    def __init__(self, tab, ctx):
        ...
        self.import_btn = Button(self.tab, text='Import Audio', command=self.import_file, width=8)
        self.import_btn.grid(row=0, column=0)

        self.process_btn = Button(self.tab, text='Process Audio', command=self.process_audio_and_plot, state='disabled', width=8)
    ...
    def import_file(self):
        ...
        self.menu = menu
        self.dropdown = OptionMenu(self.tab, menu , *effect_map.keys())
        self.dropdown.grid(row=1, column=0)

        self.add_effect_btn = Button(self.tab, text='+', command=self.add_effect, width=8)
    ...
    def add_effect(self): # config
        ...
        effect_label = Label(self.tab, text=effect_identifier.capitalize(), font=16)
        ...
        for param_key, v in default_config.items():
            ...
            label = Label(self.tab, text=param_key)
            param_entry = Entry(self.tab, textvariable=default_value, width=5)
